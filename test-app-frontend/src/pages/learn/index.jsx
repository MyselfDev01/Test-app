...
import { useLearnMenu } from "../../hooks";
import { PagesData } from "../../utility";

const LearPage = () => {
  const menu = useLearnMenu();

  const data = PagesData.find((item) => {
    return item?.url === "/learn";
  });

  return (
    <>
      <Head>
        <title key="title">Learn Here</title>
        <meta key="og:title" property="og:title" content="learn-here" />
        <meta
          key="og:url"
          property="og:url"
